...
| GATE-UI-VISUAL-001 | UI visual proof allowed | UI-011A–I, UI-012B–F, peer review, `npm run check:route` | capability + route smoke; owner sign-off pending | `ui-012f-final-visual-readiness-gate-receipt.md`, `app-peer-review-plan-alignment-2026-06-13.md` | owner UI-003E PASS | mark visual proof complete, PR ready-for-review | partial | RISK-002 | after owner UI-003E PASS |
...
| GATE-GMAIL-METADATA-001 | Gmail metadata/read-only adapter allowed | GMAIL-001B/C, GMAIL-002A hardening, GMAIL-002B gate docs, GMAIL-HARDEN-001 + `tools/gmail/` adapter | privacy/scope plan, adapter CLI, token gitignore/0600, wipe, blocked methods, read-only body gate, metadata live proof | `docs/ui/reviews/gmail-harden-001-privacy-hardening-receipt.md` | npm audit follow-up remains; draft write/send still blocked | draft write, send, token in localStorage/repo, unreviewed snapshot expansion | pass for metadata/read-only local adapter; provider writes blocked | RISK-004, RISK-010 | npm audit review or GMAIL-001D draft-write spike |
