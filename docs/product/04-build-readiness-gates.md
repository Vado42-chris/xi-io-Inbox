...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| GATE-LOCAL-OPERABILITY-001 | Tier 1 local operability allowed | UI-005A architecture docs, local operability contract, UI-005B–I receipts | Tier 1/Tier 2 boundary explicit; no runtime claims in Tier 1 | `docs/ui/reviews/ui-005i-account-session-receipt.md` | none for Tier 1 scope | Tier 2 provider/runtime actions in Tier 1 UI | pass | RISK-004, RISK-009 | UI-005I pass |
...
| GATE-UI-VISUAL-001 | UI visual proof allowed | UI-011A–I, UI-012B–F, UI-013A/B, UI-014B, peer review, `npm run check:route` | capability + route smoke + scope lens; **owner UI-013C + UI-003E PASS pending** | `ui-013-level-2-visual-experience-system.md`, `pr-20-full-peer-review-2026-06-14.md` | owner rejected UI-013B visual quality 2026-06-14 | mark visual proof complete, PR ready-for-review | blocked | RISK-002 | after UI-013C + owner UI-003E PASS |
| GATE-DRAFT-WORKBENCH-001 | Draft workbench implementation allowed | UI-007A–C receipts | 4-column shell, drafts, approval queue, simulate send | `docs/ui/reviews/ui-007c-send-event-dry-run-receipt.md` | owner proof + north-star ratification pending | provider send in Tier 1 | partial | RISK-002 | owner UI-003E PASS |
...
| GATE-GMAIL-METADATA-001 | Gmail metadata/read-only adapter allowed | GMAIL-001B/C, GMAIL-002A hardening, GMAIL-002B gate docs, GMAIL-HARDEN-001 + `tools/gmail/` adapter | privacy/scope plan, adapter CLI, token gitignore/0600 on POSIX FS, wipe, blocked methods, read-only body gate, metadata live proof | `docs/ui/reviews/gmail-harden-001-privacy-hardening-receipt.md` | draft write/send still blocked; token mode unenforceable on exFAT/NTFS | draft write, send, token in localStorage/repo, unreviewed snapshot expansion | pass for metadata/read-only local adapter; provider writes blocked | RISK-004, RISK-010 | GMAIL-001D draft-write spike |
...
| RISK-010 | Real Gmail metadata spike leaks bodies, tokens, or private fixtures into repo. | critical | medium | GMAIL-001B redaction policy, GATE-GMAIL-METADATA-001, OS keychain storage, wipe command, no localStorage tokens. | active | GATE-GMAIL-METADATA-001 |
