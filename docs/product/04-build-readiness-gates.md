...
| GATE-UI-VISUAL-001 | UI visual proof allowed | UI-011A capability matrix, UI-011B–I repairs pending, UI-003E proof packet | product capability bar met per matrix, owner review ready | `docs/product/ui-011a-product-capability-gap-matrix.md` | owner visual sign-off blocked until UI-011B–I | mark visual proof complete, PR ready-for-review | partial | RISK-002 | after UI-011B–I + owner UI-003E PASS |
| GATE-DRAFT-WORKBENCH-001 | Draft workbench implementation allowed | UI-007A–C receipts | 4-column shell, drafts v3, approval queue, rail modes, simulate send | `docs/ui/reviews/ui-007c-send-event-dry-run-receipt.md` | owner visual proof pending | provider send in Tier 1 | partial | RISK-002 | owner Inbox screenshot PASS |
...
| GATE-GMAIL-METADATA-001 | Gmail metadata-only adapter allowed | GMAIL-001B/C docs + `tools/gmail/` adapter | privacy/scope plan, adapter CLI, token gitignore, wipe, blocked methods, owner approval before primary metadata test | `docs/providers/gmail/gmail-001c-metadata-adapter-receipt.md` | owner OAuth client + metadata smoke pending | body export, draft write, send, token in localStorage/repo | partial (impl ready) | RISK-004, RISK-010 | owner metadata smoke PASS |
