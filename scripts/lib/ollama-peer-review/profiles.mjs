...
export const SLICE_PROFILES = {
  ...
  'runtime-002c': {
    id: 'RUNTIME-002C',
    title: 'RUNTIME-002C — Peer Review',
    implementationReceipt: 'docs/ui/reviews/runtime-002c-refresh-loop-operator-proof-receipt.md',
    boundaryDoc: 'docs/architecture/runtime-store-boundary-v1.md',
    priorDecisionToken: 'RUNTIME_002C_PASS_READY_FOR_OPERATOR_PROOF_AND_PEER_REVIEW',
    passDecisionToken: 'RUNTIME_002C_PEER_REVIEW_PASS_READY_FOR_DOWNSTREAM',
    partialDecisionToken: 'RUNTIME_002C_PEER_REVIEW_PARTIAL_FIXES_REQUIRED',
    nextPass: 'Downstream runtime ingress / IBAL-001 (when gated)',
    modelCloud: 'qwen3-coder:480b-cloud',
    modelLocal: 'qwen3-coder:30b',
    checks: [
      'npm run gate:runtime002c -- --write-evidence',
      'npm run check:runtime002c',
      'npm run check:runtime002b',
      'cargo test --manifest-path src-tauri/Cargo.toml',
    ],
    excludedScope: [
    ...
    reviewSections: [
      { heading: 'Refresh loop result', focus: '60s poll; read-only refreshRuntimeMail; pause when busy/hidden/disconnected.' },
      { heading: 'Manual refresh result', focus: 'runtime-refresh-now action; updates lastRefreshAt.' },
      { heading: 'Automated gate result', focus: 'gate:runtime002c unit tests + structural checks + evidence file.' },
      { heading: 'Live OAuth marker result', focus: 'Optional secrets/runtime-002c-oauth-consent.complete — not required for structural PASS.' },
      { heading: 'Static fallback result', focus: 'Loop gated to Tauri only.' },
      { heading: 'Egress gate result', focus: 'No body/draft/send/mutation exposure.' },
    ],
    files: [
      { path: 'public/src/runtime/gmail-runtime-refresh-loop.js', focus: 'Refresh loop module' },
      { path: 'scripts/test/runtime-002c-refresh-loop.test.mjs', focus: 'Unit tests' },
      { path: 'scripts/gate-runtime-002c.mjs', focus: 'Automated gate runner' },
      { path: 'docs/ui/reviews/runtime-002c-automated-gate-evidence.md', focus: 'Gate evidence output' },
      { path: 'docs/operations/automated-gates-runbook.md', focus: 'Gate policy' },
      {
        path: 'public/inbox-preview.js',
        focus: 'Refresh loop wiring (extracted)',
        extractPatterns: [
      ...
      { path: 'public/src/runtime/gmail-runtime-bridge.js', focus: 'refreshRuntimeMail read-only bundle' },
      { path: 'scripts/runtime-002c-model-check.mjs', focus: 'Structural guard' },
      { path: 'docs/ui/reviews/runtime-002c-refresh-loop-operator-proof-receipt.md', focus: 'Scope alignment' },
    ],
  },
  generic: {
    id: 'GENERIC',
